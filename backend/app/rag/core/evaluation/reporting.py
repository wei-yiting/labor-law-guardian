...
from backend.app.rag.core.embedding.baai_bge_m3_embedding import BGE_M3_MODEL
...
def save_text_report(
    results: Dict[str, Any], version: str, description: str, project_root: str
):
    project_root_path = Path(project_root)
    output_dir = project_root_path / "backend/experiments"
...
"""
    ...
    if not report_path.exists():
        ...
        # Capture the entire block for this version, from start of its header to next header or EOF.
        pattern = re.compile(
            rf"(^## Version {re.escape(version)}.*?{next_header_lookahead})",
