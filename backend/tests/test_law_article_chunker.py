import unittest
from app.rag.types import RawLawArticle, SplitStrategyEnum
from app.rag.core.ingestion.components.chunker import LawArticleChunker
...
class TestLawArticleChunker(unittest.TestCase):
    ...
    def test_case_3_contextual_strategy_subparagraphs_only(self):
        ...
        self.assertEqual(
            c1.text, "本法用詞，定義如下：一、勞工：指受雇主僱用從事工作獲致工資者。"
        )
        ...
        self.assertEqual(
            c2.text,
            "本法用詞，定義如下：二、雇主：指僱用勞工之事業主、事業經營之負責人或代表事業主處理有關勞工事務之人。",
        )
    ...
    def test_case_5_recursive_strategy(self):
        ...
        self.assertEqual(
            c1.text,
            "(1)有下列各款規定行為之一者，處新臺幣二萬元以上一百萬元以下罰鍰：一、違反第二十一條第一項、第二十二條至第二十五條...規定。",
        )
        self.assertEqual(
            c1.metadata.split_strategy, SplitStrategyEnum.numeric_contextual
        )
        self.assertEqual(c1.metadata.hierarchy.paragraph, 1)
        self.assertEqual(c1.metadata.hierarchy.subparagraph, 1)
        ...
        self.assertEqual(
            c2.text,
            "(1)有下列各款規定行為之一者，處新臺幣二萬元以上一百萬元以下罰鍰：二、違反主管機關依第二十七條限期給付工資...命令。",
        )
