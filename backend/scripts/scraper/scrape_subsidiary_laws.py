...
from scripts.scraper.law_scrape_utils import HEADERS, scrape_law_by_pcode
...
LAW_MAPPING = {
    ...
    "勞工請假規則": {"filename": "labor_leave_rule.json", "id_prefix": "LEAVE_RULE"},
...
OUTPUT_DIR = os.path.join(
    os.path.dirname(__file__), "..", "backend", "data", "law_data", "subsidiary_laws"
)
...
def main():
    ...
    for law in laws_list:
        name_clean = re.sub(
            r"（.*?）", "", law["name"]
        ).strip()  # Remove date/status info often in parens
